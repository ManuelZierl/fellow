...
def main():
    ...
    log_message(config, name="Instruction", color=0, content=introduction_prompt, language="txt")
    ...
    log_message(config, name="Instruction", color=0, content=first_message, language="txt")

    ai_response = openai_client.chat(first_message)
    log_message(config, name="AI", color=1, content=ai_response, language="json")

    while True:
        prompt_response = command_client.run(ai_response)
        print("AI:", prompt_response)
        log_message(config, name="Output", color=2, content=prompt_response, language="txt")
