...
def test_pip_install_failure(mock_run):
    ...
    mock_run.side_effect = subprocess.CalledProcessError(returncode=1, cmd='pip install numpy',
                                                         stderr='ERROR: Package not found.')

    result = pip_install(PipInstallInput(package_name='nonexistentpackage'), None)

    assert result == 'Failed to install nonexistentpackage. Error: ERROR: Package not found.'
